---
title           : "Personality Predictors of Dementia Onset and Neuropathic Burden: A Mega-Analysis"
...
1. Do personality and subjective well-being indicators (positive affect, negative affect, and satisfaction with life) predict dementia onset?  
...
Sample sizes vary bt year from 353 participants (1994) to 1487 participants, including 797 deceased participants with autopsy data (2019, 2010). This provides 99\% power to detect a zero-order correlation effect size of ~.11, two-tailed at alpha .05.  
...
**(3) Background variables** (age, gender, race, education, self-rated health, cognitive ability, cognitive impairment (MMSE))  
...
sprintf("%s/codebooks/tables.xlsx", wd) %>%
  ...
  kableExtra::group_rows("Cognitive Tests", 27, 41)
```
...
First, to test whether personality predicts dementia onset, we will run two separate models because diagnosis is a categorical (0 = no diagnosis, 1 = diagnosis) while biomarkers are continuous.
...
- Personality and well-being will be scaled as POMP across studies.  
- Education will be centered at 12 (high school graduate).  
...
We will examine four moderators of personality predicting dementia onset and status: age (in years, centered within-study), gender, education (in years, centered at 12, i.e. high school), and baseline cognitive ability (scaled as POMP).  
