...
save(hrs.rand, hrs.paq, file = sprintf("%s/data/clean/hrs_raw.RData", load_path))
...
```{r hrs load, echo = F}
load(sprintf("%s/data/clean/hrs_cleaned.RData", local_path))
```
...
```{r hrs combined print}
hrs_combined
```

```{r hrs save, eval = F}
save(hrs_cov, hrs_alpha, hrs_pers, hrs_out, hrs_combined, hrs_cog,
     file = sprintf("%s/data/clean/hrs_cleaned.RData", load_path))
```
...
```{r map load, echo = F}
load(sprintf("%s/data/clean/radc-map_cleaned.RData", local_path))
```
...
```{r ros load, echo = F}
load(sprintf("%s/data/clean/ros_cleaned.RData", local_path))
```
...
```{r eas load, echo = F}
load(sprintf("%s/data/clean/eas_cleaned.RData", local_path))
```
